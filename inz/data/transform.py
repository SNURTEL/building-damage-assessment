...
class ResizeCrop(torch.nn.Module):
    def __init__(
        self, p: float, crop_size: int, resize_size: int, weights: Sequence[float] = [1, 9.04788032, 8.68207691, 12.9632271]
    ) -> None:
        """ResizeCrop class.
        ...
        Args:
            ...
            crop_size: Size of the input image after cropping.
            resize_size: Size of the input image after resizing.
    ...
    def forward(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        if random.random() > self.p:
            crop_size = random.randint(int(self.crop_size / 1.15), int(self.crop_size / 0.85))
            ...
            for _ in range(try_cnt):
                ...
                _sc = (
                    msk[2, y0 : y0 + crop_size, x0 : x0 + crop_size].sum() * self.w[1]
